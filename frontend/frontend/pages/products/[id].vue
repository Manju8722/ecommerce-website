<template>
  ...
  <div v-else class="h-[80vh] md:mx-8 mx-3 my-2 bg-white">
    <div class="w-full mx-auto px-4 sm:px-6 lg:px-0 bg-white lg:py-12">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 mx-auto max-md:px-2">
        <div>
          <div class="img-box max-lg:mx-auto px-5 my-4">
            <img
              :src="product.image"
              alt="Yellow Tropical Printed Shirt image"
              class="object-contain aspect-[3/2] h-96 mix-blend-plus-darker"
            />
          </div>
        </div>
        <div
        ...
        >
          <div class="data w-full max-w-xl">
            <p class="text-lg font-medium leading-8 text-emerald-500 mb-4">
              {{ product.category }}
            </p>
            <h2
...
// const { data: product, pending: fetchingStatus } = await useFetch(
...
const { data: product, pending: fetchingStatus } = await useHttpGet(
  `http://localhost:5000/api/v1/get-product/` + id
);
